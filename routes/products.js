...
exports.getAllByLib = function (req, res) {
    ...
    db.collection('products', function (err, collection) {
        collection.count({ lib: { "$search": req.params.req } }, function (err, count) {
            ret.count = count;
            console.log(req.params.req + " " + count);
            collection.find({ lib: { "$search": req.params.req } }).skip(skip).limit(limit).toArray(function (err, items) {
                ret.items = items;
                res.send(ret);
            });
        });
